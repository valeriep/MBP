...
function afficherContenueNouvellePiste(pisteAModifier) {
	// Refreshir la page avec jquery mobile
	...
	// Attendre les fonctions listPaysAll(), listMassifAll.... qu'elles terminent leurs travailles
	setTimeout(
		function(){
			...
			if(pisteAModifier != null){
				...
				if(pisteAModifier.couleur != "null"){
					$('#couleur option[data-id="'+pisteAModifier.couleur+'"]').prop("selected", true);
...
function modifierDetailNouvellePiste(idPiste){
	recupererDetailNouvellePiste(idPiste);
}