...
public class FileTransfer extends CordovaPlugin {
    ...
    /**
     * Adds an interface method to an InputStream to return the number of bytes read from the raw stream. This is used to track total progress against the HTTP Content-Length
    ...
    /**
     * Provides raw bytes-read tracking for a GZIP input stream. Reports the total number of compressed bytes read from the input, rather than the number of uncompressed bytes.
     */
     ...
     * 
     *            args[2] fileKey Name of file request parameter args[3] fileName File name to be used on server args[4] mimeType Describes file content type args[5] params
        ...
        cordova.getThreadPool().execute(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    /*
                     * Store the non-file portions of the multipart data as a string, so that we can add it to the contentSize, since it is part of the body of the HTTP request.
                     */
    ...
    /**
     * This function will install a trust manager that will blindly trust all SSL certificates. The reason this code is being added is to enable developers to do development using
     * self signed SSL certificates on their web server.
     * 
     * The standard HttpsURLConnection class will throw an exception on self signed certificates if this code is not run.
     */
