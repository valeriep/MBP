...
test("constructor throws error when user not instanceof SeolanUser", function() {
    throws(function() {
        new mbp.AuthenticationService('http://test.test/', undefined);
    });
    throws(function() {
        new mbp.AuthenticationService('http://test.test/', Object.create());
    });
});
test("create login data", function() {
    var user = new mbp.SeolanUser('ch4mp', 'toto');
    var auth = new mbp.AuthenticationService('http://test.test/', user);
...
asyncTest("failed login because of bad password", function() {
    expect(3);
    var user = new mbp.SeolanUser('ch4mp', 'bad');
    var auth = new mbp.AuthenticationService('http://test.test/', user);
...
asyncTest("failed login because of missing password", function() {
    expect(3);
    var user = new mbp.SeolanUser('ch4mp', undefined);
    var auth = new mbp.AuthenticationService('http://test.test/', user);
...
asyncTest("failed login because of missing user name", function() {
    expect(3);
    var user = new mbp.SeolanUser(undefined, 'toto');
    var auth = new mbp.AuthenticationService('http://test.test/', user);
